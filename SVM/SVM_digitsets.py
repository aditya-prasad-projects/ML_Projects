from Features.mnist import load_mnist
from Features.HAAR_feature import HAAR
import numpy as np
...
class SVM_Digits:
    def read_data(self):
        with zipfile.ZipFile("../data/Digits_dataset_50_percent.zip","r") as zip_ref:
            zip_ref.extractall("./")
...
def parallel(digits, X_train, Y_train, X_test, Y_test, i):
    ...
    svm_smo = SMO(C=1, tol=0.01, max_passes=100, epochs=100)
...
X, Y = read_mnist()
haar = HAAR(100)
features = haar.get_features(X)
np.savetxt("./features30.txt", features)
np.savetxt("./labels30.txt", Y)
