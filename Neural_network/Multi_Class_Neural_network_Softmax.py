...
def loss(y_pred, Y):
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels= tf.transpose(Y), logits=tf.transpose(y_pred)))
    return cost
...
def call_everthing():
    ...
    run(X_train, Y_train ,X_test, Y_test, nodes, 2, 1e-4, 40000)
