...
def fit(X_train, Y_train, X_test, Y_test, ridge):
    if(ridge):
        regression = LogisticRegression(penalty='l2', max_iter=1000, solver= "liblinear")
    else:
        regression = LogisticRegression(penalty='l1', max_iter= 1000, solver="liblinear")
...
def read_data():
    X_train = pd.read_csv("../data/spam_polluted/train_feature.txt", sep="\s", header=None)
    Y_train = pd.read_csv("../data/spam_polluted/train_label.txt", header=None)
    X_test = pd.read_csv("../data/spam_polluted/test_feature.txt", sep="\s", header=None)
    Y_test = pd.read_csv("../data/spam_polluted/test_label.txt", header=None)
...
def run(normalise, ridge, pca):
    ...
    fit(X_train, Y_train, X_test, Y_test, ridge)

run(normalise=False, ridge = True, pca =True)
