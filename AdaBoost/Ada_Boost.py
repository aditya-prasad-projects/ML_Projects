...
class Preprocess:
    ...
    def run(self, normalise, epochs, random_classifier, k_split, database):
        with open("HW_5.2_vote_data.txt", "w") as f:
            ...
            for i in range(k_split):
                ...
                training_accuracy_temp, testing_accuracy_temp = boosting.boost(data, np.c_[X_test,  Y_test],  epochs, f)
                training_accuracy += training_accuracy_temp
                testing__accuracy += testing_accuracy_temp
            f.write("\nAverage_training_accuracy = " + str(training_accuracy / k_split))
            f.write("\nAverage_testing_accuracy = " + str(testing__accuracy  / k_split))
            print("Average_training_accuracy = ", training_accuracy / k_split)
            print("Average testing accuracy = ", testing__accuracy / k_split)
            c = []

boost = Preprocess()
boost.run(normalise = False,epochs = 201, random_classifier= False, k_split= 10, database = 1)
