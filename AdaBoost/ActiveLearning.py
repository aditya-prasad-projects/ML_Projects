...
from AdaBoost.Algorithm.AdaBoost_Active_Learning import AdaBoost_Active_Learning
...
class Preprocess:
    ...
    def run(self, normalise, epochs, random_classifier, database):
        ...
        with open("HW_5.3", "w") as f:
            ...
            for j in range(len(random_percentages)):
                train_size = int(data.shape[0] * (random_percentages[j] / 100))
                ...
                i = 0
                if (random_percentages[j] < 50):
                    end_point = int(data.shape[0] * 0.5)
                elif(random_percentages[j]< 80):
                    end_point = int(data.shape[0] * 0.8)
                else:
                    end_point = int(data.shape[0])
                while(len(training_indices) <= end_point):
                    if (random_classifier):
                        boosting = AdaBoostRandom()
                    else:
                        boosting = AdaBoost_Active_Learning()
                    training_accuracy, testing_accuracy, testing_prediction = boosting.boost(training_data, testing_data,  epochs, f)
                    f.write("\n training accuracy at " + str(int((random_percentages[j] / 100) + i)) + " = " + str(training_accuracy))
                    f.write("\n testing accuracy at " + str(int((random_percentages[j] / 100) + i)) + " = " + str(testing_accuracy))
