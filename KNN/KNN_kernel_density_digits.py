...
class KNN:
    ...
    def gaussian(self, x):
        ...
        return np.exp((-(np.linalg.norm(self.X_train - x, axis = 1)) / (2 * (0.1 ** 2))))

    def poly(self,x):
        return np.square(np.sum(np.multiply(self.X_train, x), axis = 1) + 1)
    ...
    def get_minimum_distance(self, x):
        ...
        for i in range(10):
            ...
            prob_Z_given_C = np.sum(distances[indices]) / total_sum
            prob_C_given_Z = prob_Z_given_C * (len(indices) / self.Y_train.shape[0])
            pred.append(prob_C_given_Z)
        return float(np.argmax(np.array(pred)))
